# Musterdokument — Folie für Folie

Neutrale Vorlage. Alle unternehmensbezogenen Angaben stehen als Platzhalter in
‹spitzen Klammern›. Nichts hier ist Inhalt, den man übernehmen darf — es ist die Form, in
die der recherchierte Inhalt gegossen wird.

Konventionen: `[Titel]` = `rolle:titel` · `[Kolumne]` = `rolle:kolumne` ·
`[Subline]` = `rolle:subline` · `[Quellen]` = `rolle:quelle` · `[Fußnoten]` =
`rolle:fussnote` · `[Bereich]` = Bereichsüberschrift. Die Rollen statt Indizes, weil sich
die Indizes mit dem Template-Wechsel 09/2026 geändert haben (siehe `references/powerpoint.md`).

Vorlage ist das grüne Haus-Template, Referenzdatei `20260907_Metzler_DVV_vf.pptx`.

---

## 1 — Titelfolie

```
Vertraulichkeitsvermerk:  Streng vertraulich
[Titel]                   Überlegungen zur Kapitalflexibilisierung durch Einbindung
                          privater Investoren
Untertitel (optional)     Unterlage für
                          ‹Anrede Vorname Nachname›
                          ‹Anrede Vorname Nachname›
Ort / Datum               ‹Sitz des Adressaten› / ‹Sitz des Hauses›, ‹TT. Monat JJJJ›
```

## 2 — Agenda

```
Layout   Agenda 2 — die Agenda ist eine Tabelle
[Titel]  Agenda
  01 │ ‹Haus› Corporate Finance – Vorstellung und Referenzen │ ‹Seite›
  02 │ ‹Gesellschaft› – Übersicht und Herausforderungen      │ ‹Seite›
  03 │ Diskussion möglicher Handlungsoptionen                │ ‹Seite›
```

Die Seitenzahlen in der rechten Spalte zeigen auf die Kapiteltrenner und müssen nach jedem
Einfügen oder Löschen einer Folie nachgezogen werden.

Die drei Kapitelnamen müssen **wortgleich** auf den Trennerfolien und in jeder
Kapitelkolumne (`rolle:kolumne`) stehen.

## 3 / 11 / 18 — Kapiteltrenner

```
Layout                    Kapitel 2
Kapitelnummer (ph:10)     01 | 02 | 03
Kapiteltitel  (ph:0)      ‹Kapitelname wie in der Agenda›
```

---

## Kapitel 1

## 4 — Ihre Ansprechpartner

```
[Titel]     Ihre Ansprechpartner
[Kolumne]   1. ‹Kapitelname 1›

Je Person:  ‹Name› · ‹Funktion› · ‹Bereich› · ‹Telefon› · ‹E-Mail›
Profil
  ▪ ‹Erfahrungsjahre› Jahre Erfahrung in ‹Bereich›
  ▪ Seit ‹Jahr› ‹Rolle im Haus›; Sektor ‹Fokussektor›
  ▪ Zuvor ‹Stationen›
  ▪ Ausgewählte Referenzprojekte
      – Beratung ‹Partei› bei ‹Transaktion›
      – Beratung ‹Partei› beim Verkauf ‹Asset›
```

## 5 — Das Haus

```
[Titel]   ‹Haus› – ‹Positionierung in einem Halbsatz›
  ▪ ‹Alleinstellung Eigentümerstruktur›
  ▪ ‹Netzwerk zu Eigentümern, Unternehmen, Investoren, Institutionen›
  ▪ Unabhängigkeit als zentrales Element des Geschäftsmodells
  ▪ Fokus auf nachhaltige und langfristige Kundenbeziehungen
  ▪ ‹Wertehaltung›
```

## 6 — Geschäftsfelder · 7 — Sektorfokus · 8 — Track Record

```
6  [Titel]  ‹Haus› konzentriert sich auf seine Stärken in ‹n› Geschäftsfeldern
            Spalten mit je 4–6 Leistungen
7  [Titel]  Energy & Infrastructure ist einer von ‹n› Fokussektoren
            Je Sektor 3 Unterthemen
8  [Titel]  Tiefes Branchenwissen, belegt durch den Track Record
   [Bereich] Aktuelle Mandate
            Je Tombstone: Mandatstyp · Projektname/Partei · Beschreibung in 2–3 Zeilen
```

## 9 / 10 — Case Studies

```
[Titel]    Case Study: Beratung ‹Partei› bei ‹Transaktion›
[Bereich]  Transaktion
  ▪ ‹Was verkauft/erworben wurde, Struktur, Besonderheiten›
  ▪ ‹Größenordnung, Kunden, Assets›
  ▪ ‹Ansprache von ca. ‹n› nationalen und internationalen Investoren›
[Bereich]  Rolle ‹Haus›
  ▪ ‹Haus› unterstützte ‹Partei› als exklusiver M&A-Berater
  ▪ Insbesondere folgende Leistungen:
      – Mitgestaltung des Carve-outs zur Optimierung der Equity Story
      – Strukturierung der Transaktion und Entwicklung des Investment Case
      – Erstellung von Vermarktungsunterlagen, Prozessdokumenten und Finanzmodell
      – Entscheidungsvorlagen sowie Empfehlungen zu Verhandlungsstrategie und -taktik
      – Koordination des Investorenprozesses, Organisation und Steuerung der Due Diligence
      – Unterstützung bei Verhandlungen mit der Gegenseite
      – Beratung bei kaufmännischen Aspekten des Kaufvertrages
```

---

## Kapitel 2

## 12 — Unser Verständnis der Ausgangslage

```
[Titel]    ‹Aussagesatz: Stärke + Investitionsprogramm in einem Satz›
[Bereich]  Unser Verständnis der Ausgangslage
[Kolumne]  2. ‹Gesellschaft› – Übersicht und Herausforderungen
[Quellen]  Quellen: eigene Recherche, Jahresabschluss ‹Gesellschaft› ‹Jahr›,
           Jahresabschluss ‹Tochter› ‹Jahr›, Unternehmenswebsite

Kernaussagen-Kasten:
  Die Umsetzung des Investitionsprogramms von ‹Summe› bei zeitgleicher Wahrung
  finanzieller und strategischer Flexibilität ist eine zentrale Herausforderung
  der ‹Gesellschaft›

[Bereich]  Ausgangslage
  ▪ ‹Gesellschaft› ist ‹Marktstellung› und versorgt ‹n› Kunden mit ‹Sparten›
  ▪ Das Geschäftsmodell verbindet ‹Wertschöpfungsstufen›. ‹Netzgeschäft ist gebündelt in …›
  ▪ Mit einem Umsatz von ‹X› und einem ‹Ergebnisgröße› von ‹Y› ‹Plan-Bezug›
  ▪ ‹Gesellschaft› strebt Klimaneutralität ‹Bezug› bis ‹Jahr› an
  ▪ ‹Gesellschaft› ist kommunaler Partner für die Wärmeplanung in ‹Region›
  ▪ ‹Zeitraum› sind Investitionen von ‹Summe› in ‹Schwerpunkte› vorgesehen
  ▪ ‹Gesellschaft› hat Verpflichtungen gegenüber ‹Gesellschafter/Kommune›
```

## 13 — One Pager / Steckbrief

```
[Titel]    ‹Aussagesatz zum Unternehmen: Marktstellung und Substanz in einem Satz›
[Bereich]  One Pager
[Fußnoten] 1) ‹Abweichung Abschluss/Website, z. B. Mitarbeiterzahl›;
           2) ‹Bezugsgesellschaft und Stichtag der Netzlängen›

Tabelle Merkmal | Angabe — feste Zeilenfolge:
  Firma / Sitz                │ ‹Firma›, ‹Straße›, ‹PLZ Ort›
  Rechtsform / Register       │ ‹Rechtsform›, ‹HRB› (‹Amtsgericht›)
  Aktionäre                   │ ‹Gesellschafter A› ‹X› % · ‹Gesellschafter B› ‹Y› %
  Grundkapital                │ ‹X› Mio. €
  Vorstand                    │ ‹Name› (Vors.), ‹Name›
  AR-Vorsitz                  │ ‹Name, Funktion›
  Mitarbeiter                 │ Ø ‹X› (‹Jahr›)
  Kunden                      │ ~‹X› (‹Sparten›, ‹Jahr›)
  Umsatz ‹J-1› / ‹J›          │ ‹X› / ‹Y› Mio. €
  EBT ‹J-1› / ‹J›             │ ‹X› / ‹Y› Mio. €
  Jahresüberschuss ‹J-1›/‹J›  │ ‹X› / ‹Y› Mio. €
  Eigenkapital / EK-Quote     │ ‹X› Mio. € / ‹Y› % (‹Stichtag›)
  Absatz ‹J›                  │ ‹Sparte› ‹X› GWh · ‹Sparte› ‹Y› GWh · ‹Sparte› ‹Z› Mio. m³
  Netz                        │ ‹Sparte› > ‹X› km; ‹weitere›; ‹Sparte› via ‹Tochter›
  Ergebnisverwendung          │ ‹EAV mit …› · ‹Ausgleichszahlung an …›
  Steuerungskennzahlen        │ ‹finanzielle›; nicht-finanziell: ‹operative›
```

## 14 — Beteiligungsstruktur

```
[Titel]    ‹Gesellschaft› ist kommunal und strategisch verankert und verfügt über
           ‹Charakterisierung des Portfolios›
[Bereich]  Beteiligungsstruktur
[Fußnoten] 1) Unmittelbare Beteiligungsquoten gemäß Anteilsbesitz zum ‹Stichtag›,
           weitere geringfügige Beteiligungen bestehen. Zuordnung der Geschäfts-
           bereiche auf Basis der beschriebenen Tätigkeit durch ‹Haus›

Ebene 1  Gesellschafter mit Quoten
Ebene 2  Beteiligungen mit Quote, farbkodiert nach Geschäftsbereich
Kästen bei wesentlichen Beteiligungen:
           ‹Tätigkeit in einem Halbsatz›
           Umsatz ‹J›: ‹X› Mio. EUR
           ‹Ergebnis/Gewinnabführung› ‹J›: ‹X› Mio. EUR
Legende  ‹Geschäftsbereiche›
```

## 15 — Investitionshorizont

```
[Titel]    ‹Gesellschaft› ist finanziell ‹Einordnung› − ‹Investitionsbedarf› bis ‹Jahr›
[Bereich]  Investitionen ‹J1›–‹J5› (Mio. EUR)  |  Entwicklung der Verschuldung  |
           Geplante Investitionen ‹Zeitraum›
[Fußnoten] 1) Keine öffentliche quantitative Aufteilung nach Investitionsschwerpunkten
           verfügbar; 2) Net Debt = ‹Definition›

Historie Säulen Gesamtinvestitionen ‹J1›–‹J5› · Schwerpunkte des jüngsten Jahres
Links   Säulen Net Debt ‹J1›–‹J5› · darüber Net Debt / EBITDA als ‹x›-Faktoren
Kasten  Die Verschuldung bleibt mit ‹Faktor› tragfähig, reduziert jedoch den
        finanziellen Spielraum für das Investitionsprogramm von ‹Summe›
Rechts  Zeile "Investitionsschwerpunkt": ‹A› | ‹B› | ‹C›
        Zeile "Geplante Projekte": je 3–4 Bullets mit konkretem Merkmal
        Zeile "CapEx": ‹Summe über alle Schwerpunkte›
```

## 16 — Financials

```
[Titel]    ‹Aussage zur Ertragsentwicklung› − jedoch ‹Aussage zu Cashflow/Verschuldung›
[Bereich]  Ertragslage (Mio. EUR) | Finanzlage (Mio. EUR) | Vermögenslage (Mio. EUR)
[Fußnoten] 1) ‹Definition Net Debt›; 2) ‹Erläuterung Fachbegriff›

Block 1  Umsatz und Rohertrag ‹J1›–‹J5› als zwei Säulenreihen
         CAGR +‹X› % p.a. · Rohertragsmarge in % über den Säulen
Block 2  Cashflow aus laufender Geschäfts-, Investitions- und Finanzierungstätigkeit
         Summe: Veränderung des Finanzmittelfonds
Block 3  Finanzmittelfonds und Net Debt · Eigenkapitalquote in % über den Säulen

Zeilenlogik durchgehend HGB: Betriebliches Ergebnis, Beteiligungsergebnis,
Zinsergebnis, Ergebnis vor Steuern — nicht EBITDA-/EBIT-/EAT-Margen.

Kommentar:
  ▪ ‹Satz zur Ertragslage›
  ▪ ‹Satz zur Volatilität der Cashflows mit Erklärung der Ausreißer›
  ▪ Das Investitionsvolumen von ‹Summe› über ‹Zeitraum› scheint nicht aus dem
    operativen Cash Flow finanzierbar zu sein
```

## 17 — Summary / Überleitung zu Kapitel 3

```
[Titel]    ‹Kernaussage des Kapitels als Aussagesatz›
[Bereich]  Summary ‹Kapitelname› / Überleitung zu Kapitel 3

Kernbotschaft (Absatz, kein Bullet):
  ‹Gesellschaft› ist ein ‹Charakterisierung› mit ‹Stärke 1›, ‹Stärke 2› und
  ‹Stärke 3› — steht aber am Beginn ‹der investitionsintensivsten Phase›.
  ‹Auslöser› machen ‹Gesellschaft› zum ‹Rolle›, während gleichzeitig
  ‹Gegenkräfte› die Innenfinanzierungskraft begrenzen.

Leitfrage (hervorgehobener Kasten, ein Satz):
  Wie finanziert und organisiert ‹Gesellschaft› ‹das Vorhaben› bei gleichzeitig
  ‹bestehender Verpflichtung› und ‹bestehender Nebenbedingung›?
```

*Optional davor:* eine Folie **Strategische Stoßrichtungen** — fünf bis sechs Themenfelder,
je ein Satz in der Sprache des Unternehmens. Sie ersetzt die Summary nicht.

---

## Kapitel 3

## 19 — Mögliche Ziele des Unternehmens

```
[Titel]    Verschiedene Handlungsoptionen für die zukünftige Positionierung ‹Gesellschaft›
[Bereich]  Mögliche Ziele des Unternehmens
[Quellen]  Quellen: eigene Recherche, Jahresabschlüsse ‹Gesellschaft› ‹J1›–‹J5›,
           Geschäftsbericht ‹J5›, Presseberichterstattung

  1. ‹Transformation: Wärmeplanung, Klimaneutralität bis ‹Jahr›, Dekarbonisierung›
  2. ‹Finanzierung des Investitionsprogramms bei EK-Quote ‹X› %›
  3. ‹Ergebnisstabilität und Absicherung der Ausschüttungsfähigkeit›
  4. ‹Sicherung knapper Ressourcen: Baukapazitäten, Fachkräfte, IT›
  5. ‹Wachstum in energienahen Geschäftsfeldern›
  6. ‹Zukunftsthemen mit laufenden Strategieprojekten›
  7. ‹Risikomanagement: Markt-, Ausfall-, Cyber-, Regulierungsrisiken›
```

Jedes Ziel mit einem konkreten Anker — Jahreszahl, Projekt, Quote oder Gremienbeschluss —
und aus Abschluss, Geschäftsbericht oder Presse abgeleitet, nicht aus einem Standardkatalog.

## 20 — Ansatzpunkte für direkte Vorschläge

```
[Titel]    ‹Aussagesatz: woran konkret gearbeitet werden könnte›
[Bereich]  Ansatzpunkte für direkte Vorschläge
[Fußnoten] 1) ‹Hinweis auf Übertragbarkeit oder regulatorische Machbarkeit›

Finanzierungsseite          │ Eigenkapitalseite
  ▪ ‹strukturierte Lang-    │   ▪ ‹Kapitalerhöhung oder Thesaurierungspfad›
    fristfinanzierung›      │   ▪ ‹Balance Ausschüttung vs. EK-Quote›
  ▪ ‹Förderkredite,         │   ▪ ‹hybride Instrumente›
    Bundesprogramme›        │   ▪ ‹EK-Ausstattung einzelner Töchter›
  ▪ ‹Schuldschein / Green   │
    Finance›                │
────────────────────────────┼──────────────────────────────────────────────
Portfolio und Struktur      │ Operativ
  ▪ ‹Bewertung und Weiter-  │   ▪ ‹Beschaffungs- und Produktstrategie›
    entwicklung Beteilig.›  │   ▪ ‹Forderungsmanagement›
  ▪ ‹Beteiligung privater   │   ▪ ‹Digitalisierung zur Kostensenkung›
    Partner an ‹Einheit››   │
  ▪ ‹Contracting / Wärme-   │
    lösungen›               │
```

Jeder Ansatz knüpft sichtbar an ein Ziel aus Folie 19 und an einen Beleg aus Kapitel 2 an.

*Optionale Vertiefungen:* Beteiligungsmodell im Schaubild (Carve-out, Investor mit
Minderheit, Governance mit Reserved Matters, Dividendenpolitik, Exit) · Optionenspektrum mit
Einschätzung inklusive Nachteilen · Investorenlandschaft institutionell gegen strategisch.

## 21 — Mehrwert des Hauses

```
[Titel]    ‹Haus› verbindet tiefes Branchen-Know-how, exzellenten Investorenzugang und
           unabhängige Transaktionsexpertise
[Bereich]  Mehrwert ‹Haus›
  ▪ Umfangreiches Know-how im Energiesektor sowie bei Transaktionen mit
    öffentlich-rechtlichem Hintergrund
  ▪ Hervorragende Reputation und exzellenter Zugang zu relevanten nationalen und
    internationalen Projekten und Investoren
  ▪ Erfahrener Sparringspartner bzgl. Transaktionsprozess, -taktik und -struktur sowie
    Erfahrung mit Entscheidungsprozessen der öffentlichen Hand
  ▪ Unternehmerisches Denken und Handeln
  ▪ Qualifiziertes und erfahrenes Team mit hohem Senior-Commitment
  ▪ Unabhängige Beratung ohne eigene Finanzierungs- oder Beteiligungsinteressen →
    keine Interessenkonflikte
```

## 22 — Rechtliche Hinweise

Standardtext aus dem Layout `Rechtliche Hinweise`. Nicht neu schreiben, nur prüfen,
dass die Kontaktangaben stimmen.
