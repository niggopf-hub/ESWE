...
Zieladressen:
    ...
    "id:98"                Shape mit dieser Shape-ID - noetig, wenn Namen mehrfach vorkommen
                           (in der gruenen Vorlage heissen alle Kacheln "Text Box 29")
...
def finde_shapes(slide, art, wert):
    ...
    for sh in alle_shapes(slide.shapes):
        if art == "ph" and sh.is_placeholder:
            ...
            treffer.append(sh)
        elif art == "id" and str(sh.shape_id) == wert:
            treffer.append(sh)
    return treffer
...
def loese_ziel(slide, adresse):
    ...
    if art not in ("ph", "name", "rolle", "id"):
        return None, None, f"'{adresse}': unbekannte Adressart (erlaubt: rolle:, ph:, name:, id:)"
    if art == "rolle":
    ...
    if len(treffer) > 1:
        return None, None, (f"'{adresse}' ist mehrdeutig: {len(treffer)} Shapes tragen "
                            "diesen Namen. Ueber id:<Shape-ID> adressieren (inspect_deck.py --slide N zeigt sie).")
    sh = treffer[0]
    if zelle:
        if not sh.has_table:
